    ...
    private animate(): void {
        ...
        if (!this.isGameOver) {
            ...
            if (this.scene.fog instanceof THREE.Fog) {
                ...
                // As the player descends, the fog gets closer and claustrophobic
                this.scene.fog.near = 20 - (depthRatio * 15); 
                this.scene.fog.far = 80 - (depthRatio * 65);  
            }

            // --- STATE TRACKING LOG SYSTEM (STEALTH ZONE) ---
            // Hidden if inside the stealth box OR physically lower than Y = -5
            const currentlyInside = this.shelter.stealthBox.containsPoint(this.player.getPosition()) || playerY < -5;
